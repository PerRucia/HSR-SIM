...
class Numby(Summon):
    ...
    def takeTurn(self) -> tuple[list[Buff], list[Debuff], list[Advance], list[Delay], list[Turn]]:
        ...
        tl.append(Turn(self.name, self.ownerRole, -1, AtkTarget.NA, [Move.ALL], [self.element], [0, 0], [0, 0], 0, self.scaling, 0, "NumbyGoGo"))
...
class Fuyuan(Summon):
    ...
    def takeTurn(self) -> tuple[list[Buff], list[Debuff], list[Advance], list[Delay], list[Turn]]:
        bl, dbl, al, dl, tl = super().takeTurn()
        tl.append(Turn(self.name, self.ownerRole, -1, AtkTarget.NA, [Move.ALL], [self.element], [0, 0], [0, 0], 0, self.scaling, 0, "FuyuanGoGo"))
        return bl, dbl, al, dl, tl