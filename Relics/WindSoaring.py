...
class WindSoaring(Relic):
    ...
    def equip(self):
        buff_lst = [(Buff("WindSoaringATK", "ATK%", 0.12, self.wearerRole, ["ALL"], 1, 1, Role.SELF, TickDown.PERM))]
        if self.setType == 4:
            buff_lst.append(Buff("WindSoaringCR", "CR%", 0.06, self.wearerRole, ["ALL"], 1, 1, Role.SELF, TickDown.PERM))
        return buff_lst, [], [], []
    
    def useFua(self, enemyID):
        buffList, debuffList, advList, delayList = super().useFua(enemyID)
        if self.setType == 4:
            buffList.append(Buff("WindSoaringDMG", "DMG%", 0.36, self.wearerRole, ["ULT"], 1, 1, Role.SELF, TickDown.END))
