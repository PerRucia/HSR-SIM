...
class Baptism(Lightcone):
    ...
    def equip(self):
        ...
        buffList.append(Buff("BaptismCD", Pwr.CD_PERCENT, cdBuff, self.wearerRole, [Move.ALL], 1, 1, Role.SELF, TickDown.PERM))
        return buffList, debuffList, advList, delayList
    
    def useBsc(self, enemyID=-1):
        bl, dbl, al, dl = super().useBsc(enemyID)
        cdBuff = (self.level * 0.01 + 0.07) * self.targetDebuffs
        bl.append(Buff("BaptismDebuffCD", Pwr.CD_PERCENT, cdBuff, self.wearerRole, [Move.ALL], 1, 1, Role.SELF, TickDown.PERM))
        return bl, dbl, al, dl
