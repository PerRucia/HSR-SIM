...
class BaptismFeixiao(Lightcone):
    ...
    def equip(self):
        ...
        buffList.append(Buff("BaptismCD", "CD%", cdBuff, self.wearerRole, ["ALL"], 1, 1, Role.SELF, "PERM"))
        return buffList, debuffList, advList, delayList
    
    def useBsc(self, enemyID=-1):
        bl, dbl, al, dl = super().useBsc(enemyID)
        cdBuff = (self.level * 0.01 + 0.07) * self.targetDebuffs
        bl.append(Buff("BaptismDebuffCD", "CD%", cdBuff, self.wearerRole, ["ALL"], 1, 1, Role.SELF, "PERM"))
        return bl, dbl, al, dl
    ...
    def useUlt(self, enemyID):
        ...
        bl.append(Buff("BaptismDispDMG", "DMG%", dmgBuff, self.wearerRole, ["ALL"], 2, 1, Role.SELF, "END"))
        shredBuff = self.level * 0.04 + 0.20
        bl.append(Buff("BaptismDispSHRED", "SHRED", shredBuff, self.wearerRole, ["FUA"], 2, 1, Role.SELF, "END"))
        return bl, dbl, al, dl
