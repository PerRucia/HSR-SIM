...
class Baptism(Lightcone):
    ...
    def equip(self):
        ...
        buffList.append(Buff("BaptismCD", Pwr.CD_PERCENT, cdBuff, self.wearerRole, [AtkType.ALL], 1, 1, Role.SELF, TickDown.PERM))
    ...
    def useUlt(self, enemyID=-1):
        ...
        bl.append(Buff("BaptismDispDMG", Pwr.DMG_PERCENT, dmgBuff, self.wearerRole, [AtkType.ALL], 2, 1, Role.SELF, TickDown.END))
        shredBuff = self.level * 0.04 + 0.20
        bl.append(Buff("BaptismDispSHRED", Pwr.SHRED, shredBuff, self.wearerRole, [AtkType.FUA], 2, 1, Role.SELF, TickDown.END))
        return bl, dbl, al, dl
