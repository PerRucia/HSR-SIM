...
class Aeon(Lightcone):
    ...
    def equip(self):
        ...
        buffList.append(Buff("AeonDMG", "DMG%", dmgBuff, self.wearerRole, ["ALL"], 1, 1, Role.SELF, TickDown.PERM))
        return buffList, debuffList, advList, delayList
    
    def useUlt(self, enemyID):
        bl, dbl, al, dl = super().useUlt(enemyID)
        atkBuff = self.level * 0.02 + 0.06
        bl.append(Buff("AeonATK", "ATK%", atkBuff, self.wearerRole, ["ALL"], 1, 4, Role.SELF, TickDown.PERM))
        return bl, dbl, al, dl
