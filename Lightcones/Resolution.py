...
class ResolutionPela(Lightcone):
    ...
    def useBsc(self, enemyID):
        ...
        dbl.append(Debuff("ResoShred", self.wearerRole, "SHRED", shredBuff, enemyID, ["ALL"], 1, 1, False, [0, 0], False))
        return bl, dbl, al, dl
    ...
    def useUlt(self, enemyID):
        ...
        shredBuff = self.level * 0.01 + 0.11
        dbl.append(Debuff("ResoShred", self.wearerRole, "SHRED", shredBuff, "ALL", ["ALL"], 1, 1, False, [0, 0], False))
        return bl, dbl, al, dl
    
class ResolutionJQ(Lightcone):
    name = "Resolution Shines as Pearls of Sweat"
    path = "NIH"
    baseHP = 952.6
    baseATK = 476.28
    ...
    def useSkl(self, enemyID):
        bl, dbl, al, dl = super().useSkl(enemyID)
        shredBuff = self.level * 0.01 + 0.11
        dbl.append(Debuff("ResoShred", self.wearerRole, "SHRED", shredBuff, enemyID, ["ALL"], 1, 1, False, [0, 0], True))
        return bl, dbl, al, dl
