...
class Blissful(Lightcone):
    ...
    def equip(self):
        ...
        buffList.append(Buff("BlissfulCR", "CR%", crBuff, self.wearerRole, ["ALL"], 1, 1, Role.SELF, TickDown.PERM))
        dmgBuff = self.level * 0.05 + 0.25
        buffList.append(Buff("BlissfulDMG", "DMG%", dmgBuff, self.wearerRole, ["FUA"], 1, 1, Role.SELF, TickDown.PERM))
        return buffList, debuffList, advList, delayList
    
    def useFua(self, enemyID):
        bl, dbl, al, dl = super().useFua(enemyID)
        cdBuff = self.level * 0.02 + 0.10
        dbl.append(Debuff(f"BlissfulTame{self.wearerRole.name}", self.wearerRole, "CD%", cdBuff, self.defaultTarget, ["ALL"], 1000, 2, False, [0, 0], False))
        return bl, dbl, al, dl
