...
class Duran(Planar):
    ...
    def allyTurn(self, turn: Turn, result: Result):
        ...
        if "FUA" in turn.atkType and turn.moveName not in bonusDMG:
            ...
            bl.append(Buff("DuranDMG", Pwr.DMG_PERCENT, 0.05, self.wearerRole, ["FUA"], 1, 5, Role.SELF, TickDown.PERM))
            if self.procs == 5 and not self.appliedCDBuff:
                self.appliedCDBuff = True
                bl.append(Buff("DuranCD", Pwr.CD_PERCENT, 0.25, self.wearerRole, ["ALL"], 1, 1, Role.SELF, TickDown.PERM))
        return bl, dbl, al, dl
