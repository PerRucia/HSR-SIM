...
class KalpagniGallagher(Planar):
    ...
    def equip(self):
        ...
        buffList.append(Buff("KalpagniSPD", "SPD%", 0.06, self.wearerRole, ["ALL"], 1, 1, Role.SELF, "PERM"))
        return buffList, debuffList, advList, delayList
    
    def useBsc(self, enemyID):
        bl, dbl, al, dl = super().useBsc(enemyID)
        if self.fireWeakEnemies:
            bl.append(Buff("KalpagniBE", "BE%", 0.40, self.wearerRole, ["ALL"], 1, 1, Role.SELF, "END"))
        return bl, dbl, al, dl
