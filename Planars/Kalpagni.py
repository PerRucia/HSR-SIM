...
class KalpagniGallagher(Planar):
    ...
    def equip(self):
        ...
        buffList.append(Buff("KalpagniSPD", Pwr.SPD_PERCENT, 0.06, self.wearerRole, ["ALL"], 1, 1, Role.SELF, TickDown.PERM))
        return buffList, debuffList, advList, delayList
    
    def useBsc(self, enemyID):
        bl, dbl, al, dl = super().useBsc(enemyID)
        if self.fireWeakEnemies:
            bl.append(Buff("KalpagniBE", Pwr.BE_PERCENT, 0.40, self.wearerRole, ["ALL"], 1, 1, Role.SELF, TickDown.END))
        return bl, dbl, al, dl
