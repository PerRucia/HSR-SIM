...
class HuoHuo(Character):
    ...
    def useBsc(self, enemyID=-1):
        ...
        tl.append(Turn(self.name, self.role, self.getTargetID(enemyID), Targeting.SINGLE, [AtkType.BSC], [self.element], [0.5, 0], [10, 0], 20, self.scaling, 1, "HuoHuoBasic"))
        return bl, dbl, al, dl, tl
    
    def useSkl(self, enemyID=-1):
        bl, dbl, al, dl, tl = super().useSkl(enemyID)
        tl.append(Turn(self.name, self.role, self.getTargetID(enemyID), Targeting.NA, [AtkType.SKL], [self.element], [0, 0], [0,0], 36, self.scaling, -1, "HuoHuoSkill"))
        return bl, dbl, al, dl, tl
    
    def useUlt(self, enemyID=-1):
        bl, dbl, al, dl, tl = super().useUlt(enemyID)
        self.currEnergy = self.currEnergy - self.ultCost
        bl.append(Buff("HuoHuoUltATK", Pwr.ATK_PERCENT, 0.4, Role.ALL, [AtkType.ALL], 2, 1, Role.SELF, TickDown.END))
        bl.append(Buff("HuoHuoERR", Pwr.ERR_F, self.ally1Energy, self.ally1Role, [AtkType.ALL], 1, 1, self.ally1Role, TickDown.PERM))
        bl.append(Buff("HuoHuoERR", Pwr.ERR_F, self.ally2Energy, self.ally2Role, [AtkType.ALL], 1, 1, self.ally2Role, TickDown.PERM))
        bl.append(Buff("HuoHuoERR", Pwr.ERR_F, self.ally3Energy, self.ally3Role, [AtkType.ALL], 1, 1, self.ally3Role, TickDown.PERM))
