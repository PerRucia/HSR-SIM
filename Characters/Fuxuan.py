...
class Fuxuan(Character):
    ...
    def __init__(self, pos: int, role: str, defaultTarget: int = -1, lc = None, r1 = None, r2 = None, pl = None, subs = None) -> None:
        ...
        self.relicStats = subs if subs else RelicStats(8, 0, 4, 4, 12, 4, 4, 4, 4, 4, 0, 0, Pwr.HP_PERCENT, Pwr.SPD, Pwr.HP_PERCENT, Pwr.ERR_PERCENT)
    ...
    def useBsc(self, enemyID=-1):
        ...
        tl.append(Turn(self.name, self.role, self.getTargetID(enemyID), Targeting.SINGLE, [AtkType.BSC], [self.element], [0.5, 0], [10, 0], 20, self.scaling, 1, "FuxuanBasic"))
    ...
    def useSkl(self, enemyID=-1):
        ...
        bl.append(Buff("FuxuanCR", Pwr.CR_PERCENT, 0.12, Role.SELF, [AtkType.ALL], 3, 1, self.role, TickDown.START))
        return bl, dbl, al, dl, tl
    
    def useUlt(self, enemyID=-1):
        bl, dbl, al, dl, tl = super().useUlt(enemyID)
        self.currEnergy = self.currEnergy - self.ultCost
        tl.append(Turn(self.name, self.role, self.getTargetID(enemyID), Targeting.AOE, [AtkType.ULT], [self.element], [1.0, 0], [20, 0], 5, self.scaling, 0, "FuxuanUlt"))
        return bl, dbl, al, dl, tl
