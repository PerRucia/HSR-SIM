...
class Feixiao(Character):
    ...
    def __init__(self, pos: int, role: Role, defaultTarget: int= -1, eidolon: int = 0, lc = None, r1 = None, r2 = None, pl = None, subs = None, rotation = None, targetPrio = Priority.DEFAULT):
    ...
    def useBsc(self, enemyID=-1):
        ...
        tl.append(Turn(self.name, self.role, self.bestEnemy(enemyID), Targeting.SINGLE, [AtkType.BSC], [self.element], [basicMul, 0], [10, 0], 0.5, self.scaling, 1, "FeixiaoBasic"))
    ...
    def useSkl(self, enemyID=-1):
        ...
        tl.append(Turn(self.name, self.role, self.bestEnemy(enemyID), Targeting.SINGLE, [AtkType.SKL], [self.element], [sklMul, 0], [20, 0], 0.5, self.scaling, -1, "FeixiaoSkill"))
        self.fuas = self.fuas + 1
        tl.append(Turn(self.name, self.role, self.bestEnemy(enemyID), Targeting.SINGLE, atkType, [self.element], [fuaMul + e6Bonus, 0], [5 + e4Bonus, 0], 0.5 + e2Bonus, self.scaling, 0, "FeixiaoSkillFUA"))
    ...
    def useUlt(self, enemyID=-1):
        ...
        for i in range(6):
            tl.append(Turn(self.name, self.role, self.bestEnemy(enemyID), Targeting.SINGLE, [AtkType.ULT, AtkType.FUA, AtkType.DUKEULT], [self.element], [boltMul * dmgMul[i], 0], [10, 0], 0, self.scaling, 0, "FeixiaoUlt"))
        tl.append(Turn(self.name, self.role, self.bestEnemy(enemyID), Targeting.SINGLE, [AtkType.ULT, AtkType.FUA, AtkType.DUKEULT], [self.element], [finalMul * dmgMul[5], 0], [0, 0], 0, self.scaling, 0, "FeixiaoUltFinal"))
        return bl, dbl, al, dl, tl
    ...
    def useFua(self, enemyID=-1):
        ...
        e6Bonus = 1.4 if self.eidolon == 6 else 0
        atkType = [AtkType.FUA, AtkType.ULT, AtkType.DUKEFUA] if self.eidolon == 6 else [AtkType.FUA, AtkType.DUKEFUA]
        tl.append(Turn(self.name, self.role, self.bestEnemy(enemyID), Targeting.SINGLE, atkType, [self.element], [fuaMul + e6Bonus, 0], [5 + e4Bonus, 0], 0.5 + e2Bonus, self.scaling, 0, "FeixiaoFua"))
    ...
    def allyTurn(self, turn: Turn, result: Result):
        ...
        if turn.targeting != Targeting.NA and turn.moveName not in bonusDMG:
            ...
            if AtkType.FUA in turn.atkType and self.eidolon >= 2 and self.e2Count > 0:
                ...
                tl.append(Turn(self.name, self.role, -1, Targeting.NA, [AtkType.ALL], [self.element], [0, 0], [0, 0], 1.0, self.scaling, 0, "FeixiaoAllyEnergy(E2FUA)"))
            else:
                tl.append(Turn(self.name, self.role, -1, Targeting.NA, [AtkType.ALL], [self.element], [0, 0], [0, 0], 0.5, self.scaling, 0, "FeixiaoAllyEnergy"))
        return bl, dbl, al, dl, tl
    ...
    def handleSpecialStart(self, specialRes: Special):
        ...
        if self.technique:
            ...
            tl.append(Turn(self.name, self.role, self.bestEnemy(-1), Targeting.AOE, [AtkType.TECH], [self.element], [2.0, 0], [20, 0], 1.5, self.scaling, 0, "FeixiaoTech"))
        elif specialRes.specialName == "Feixiao":
            if self.fuaTrigger and specialRes.attr2 and not self.firstTurn:
                tl.append(Turn(self.name, self.role, -1, Targeting.NA, [AtkType.ALL], [self.element], [0, 0], [0, 0], 0.5, self.scaling, 0, "FeixiaoPityEnergy"))
        return bl, dbl, al, dl ,tl
