...
class Moze(Character):
    ...
    def useBsc(self, enemyID=-1):
        ...
        tl.append(Turn(self.name, self.role, self.getTargetID(enemyID), Targeting.SINGLE, [AtkType.BSC], [self.element], [1.1, 0], [10, 0], 20, self.scaling, 1, "MozeBasic"))
    ...
    def useSkl(self, enemyID=-1):
        ...
        tl.append(Turn(self.name, self.role, self.getTargetID(enemyID), Targeting.SINGLE, [AtkType.SKL], [self.element], [1.65, 0], [20, 0], 30, self.scaling, -1, "MozeSkill"))
        return bl, dbl, al, dl, tl
    ...
    def useUlt(self, enemyID=-1):
        ...
        tl.append(Turn(self.name, self.role, self.getTargetID(enemyID), Targeting.SINGLE, [AtkType.ULT, AtkType.FUA, AtkType.DUKEULT], [self.element], [2.916, 0], [30, 0], 5, self.scaling, 0, "MozeULT"))
        self.fuas = self.fuas + 1
        tl.append(Turn(self.name, self.role, self.getTargetID(enemyID), Targeting.SINGLE, [AtkType.FUA, AtkType.DUKEFUA], [self.element], [1.76 + 0.25, 0], [10, 0], 10, self.scaling, spRegen, "MozeUltFUA"))
        return bl, dbl, al, dl, tl
    ...
    def useFua(self, enemyID=-1):
        ...
        if spRegen == 1:
            self.fuaRegainSP = False
        tl.append(Turn(self.name, self.role, self.getTargetID(enemyID), Targeting.SINGLE, [AtkType.FUA, AtkType.DUKEFUA], [self.element], [1.76 + 0.25, 0], [10, 0], 10, self.scaling, spRegen, "MozeFUA"))
        return bl, dbl, al, dl, tl
    ...
    def allyTurn(self, turn: Turn, result: Result):
        ...
        if turn.targeting != Targeting.NA and turn.moveName not in bonusDMG:
            ...
            if self.charge >= 0:
                tl.append(Turn(self.name, self.role, result.enemiesHit[0], Targeting.SINGLE, [AtkType.SPECIAL], [self.element], [0.33, 0], [0, 0], 2, self.scaling, 0, "MozeBonusDMG"))
