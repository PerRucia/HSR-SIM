...
class Tingyun(Character):
    ...
    def __init__(self, pos: int, role: Role, defaultTarget: int = -1, lc = None, r1 = None, r2 = None, pl = None, subs = None, eidolon = 6, beneTarget = None, rotation = None, targetPrio = Priority.DEFAULT) -> None:
    ...
    def useBsc(self, enemyID=-1):
        ...
        tl.append(Turn(self.name, self.role, self.bestEnemy(enemyID), Targeting.SINGLE, [AtkType.BSC], [self.element], [e3Mul, 0], [10, 0], 25, self.scaling, 1, "TingyunBasic"))
        tl.append(Turn(self.name, self.beneTarget, self.bestEnemy(enemyID), Targeting.SINGLE, [AtkType.SPECIAL], [self.element], [e5Mul, 0], [0, 0], 0, Scaling.ATK, 0, "TYAllyBonus"))
        return bl, dbl, al, dl, tl
    
    def useSkl(self, enemyID=-1):
        bl, dbl, al, dl, tl = super().useSkl(enemyID)
        tl.append(Turn(self.name, self.role, -1, Targeting.NA, [AtkType.SKL], [self.element], [0, 0], [0, 0], 35, self.scaling, -1, "TingyunSkill"))
    ...
    def useUlt(self, enemyID=-1):
        ...
        tl.append(Turn(self.name, self.role, -1, Targeting.NA, [AtkType.ULT], [self.element], [0, 0], [0, 0], 5, self.scaling, 0, "TingyunUlt"))
