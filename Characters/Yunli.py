...
class Yunli(Character):
    ...
    def useBsc(self, enemyID=-1):
        ...
        return bl, dbl, al, Turn(self.name, -1, "ST", ["BSC"], [self.element], [1.0, 0], [10, 0], 20, self.scaling)
    
    def useSkl(self, enemyID=-1):
        bl, dbl, al, *_ = super().useSkl(enemyID)
        return bl, dbl, al, Turn(self.name, -1, "BLAST", ["SKL"], [self.element], [1.2, 0.6], [20, 10], 30, self.scaling)
    
    def useUlt(self, enemyID=-1):
        self.cullActive = True
        bl, dbl, al, *_ = super().useSkl(enemyID)
        return bl, dbl, al, Turn(self.name, -1, "NA", [], [self.element], [0, 0], [0, 0], 5, self.scaling)
    
    def useFua(self, enemyID=-1):
        bl, dbl, al, *_ = super().useFua(enemyID)
        if self.cullActive:
            self.cullActive = False
            return bl, dbl, al, Turn(self.name, enemyID, "BLAST", ["FUA", "ULT"], [self.element], [6.52 , 1.1], [25, 10], 10, self.scaling)
        return bl, dbl, al, Turn(self.name, enemyID, "BLAST", ["FUA"], [self.element], [1.2, 0.6], [20, 10], 5, self.scaling)
